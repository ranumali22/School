...
const ClassTest = () => {
    ...
    useEffect(() => {
        ...
        getClassTest(school_id, session_id);
    }, []);

    const getClassTest = (school_id, session_id) => {
        fetch(localurl + "class_test/" + school_id + "/" + session_id)
            .then(res => res.json())
            .then(data => {
                console.log("API DATA 👉", data);
                if (data.success) {
                    const sortedData = (data.row || data.data || []).sort((a, b) => {
    ...
    const handleSubmit = async (action) => {
        ...
        if (!examData.test_name?.trim()) newErrors.test_name = "Required";
        if (!examData.test_date) newErrors.test_date = "Required";
        ...
        if (action === "edit") {
            ...
            let url = localurl + "update_class_test/" + edit_id;
            ...
            fetch(url, requestOptions)
                ...
                .then((result) => {
                    ...
                    if (data.success) {
                        ...
                        if (msg.includes("test_name")) newErrors.test_name = data.message;
                        if (msg.includes("test_date")) newErrors.test_date = data.message;
        ...
        // ================= ADD =================
            ...
            try {
                ...
                fetch(localurl + "add_class_test", requestOptions)
                    .then((response) => response.text())
                    .then((result) => {
                        const data = JSON.parse(result);
    ...
    const handleEdit = (index, id) => {
        ...
        setExamData({
            ...
            test_type: item.test_type || "Class Test",
    ...
    const toggleStatus = (id, currentStatus) => {
        ...
        fetch(localurl + "status_class_test/" + id, {
            method: "PUT",
            headers: myHeaders,
            body: JSON.stringify({
                ...
                <table className="min-w-full border">
                    <thead>
                        <tr className="bg-[#0860C4] text-center text-white">
                            ...
                            <th onClick={() => handleSort("test_name")} className="px-2 md:px-3 py-2 font-medium text-[14px] whitespace-nowrap cursor-pointer">Test Name ⬍</th>
                            <th onClick={() => handleSort("test_type")} className="px-2 md:px-3 py-2 font-medium text-[14px] whitespace-nowrap cursor-pointer">Test Type ⬍</th>
                            <th onClick={() => handleSort("test_date")} className="px-2 md:px-3 py-2 font-medium text-[14px] whitespace-nowrap cursor-pointer">Test Date ⬍</th>
                        ...
                        {test.length > 0 ? (
                            test.map((e, index) => (
                                <tr key={index} className="text-center border-t">
                                    ...
                                    <td className="p-2 ">{e.test_name}</td>
                                    <td className="p-2">{e.test_type}</td>
                                    <td className="p-2">{formatDate(e.test_date)}</td>
                ...
                <div className="fixed inset-0 bg-black/40 flex justify-center items-center z-50">
                    <div className="bg-white rounded-xl w-[600px] p-6">
                        <h2 className="text-xl font-bold text-black mb-6">
                            {ButtonAction === "edit" ? "Update Class Test" : "Add Class Test"}
                        </h2>

                        <div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <FloatingInput
                                    label={<>Test Name <span className="text-red-500">*</span></>}
                                    name="test_name"
                                    type="text"
                                    value={examData.test_name}
                                ...
                                <FloatingInput
                                    label={<>Test Date <span className="text-red-500">*</span></>}
                                    name="test_date"
                                    type="date"
