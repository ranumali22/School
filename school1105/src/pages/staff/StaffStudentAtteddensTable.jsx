...
import StaffStudentAttendance from "./StaffStudentAttendance";

const StaffStudentAttendancetable = () => {
    ...
    const handleAttendance = async (date = form.date) => {
        const session_id = localStorage.getItem("session_id");
        const authData = JSON.parse(localStorage.getItem("authData") || "{}");
        const staff_id = authData?.id;

        try {
            // 1. Fetch class details to get assigned classes for this staff
            const detailRes = await fetch(localurl + "class_detail/" + school_id);
            ...
            // 2. Fetch class attendance list
            const attRes = await fetch(localurl + "class_attendance_list/" + school_id + "/" + session_id + "/" + date);
            const data = await attRes.json();

            if (data.success) {
                console.log("data attend=>", data);

                // 3. Filter classes so staff only sees their assigned classes
                const filteredList = data.raw.filter(item => 
                    assignedClassIds.includes(String(item.class_id))
                );
...
export default StaffStudentAttendancetable;