...
const SchoolList = () => {
  ...
  const toggleActive = async (item) => {
    try {
      const newStatus = item.delete_status === "show" ? "delete" : "show";
  ...
  const handleUpdate = async () => {
    try {
      ...
      handleApiResponse(res.data);
        ...
        // logged-in school ka localStorage bhi update karo
        const authData = JSON.parse(localStorage.getItem("authData") || "{}");

        if (authData?.id == editData.id) {
          const profileRes = await get_profile_school(editData.id);

          if (profileRes.data?.row?.length > 0) {
            localStorage.setItem(
              "authData",
              JSON.stringify(profileRes.data.row[0]),
            );
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 overflow-y-auto">
          <div className="bg-white p-6 rounded-xl w-[700px] max-h-[90vh] overflow-y-auto">
            ...
            {(editData.logoPreview || editData.upload_logo) && (
              <img
                src={getLogoUrl(editData.logoPreview || editData.upload_logo)}
                className="h-20 mt-2"
              />
            )}
